...
export class ProductRestockModalComponent {
  ...
  protected readonly quantity = signal(
    this.recommendedQuantity(this.product),
  );
  protected readonly supplier = signal(
    this.isBulk ? KEEP_CURRENT_SUPPLIERS : this.product.supplier || PRODUCT_SUPPLIERS[0],
  );
  protected readonly eta = signal(this.hasOutOfStockProducts() ? '2 business days' : 'Next supplier run');
  ...
  protected readonly projectedStock = computed(() =>
    this.currentStock() + this.totalIncoming(),
  );

  protected readonly orderValue = computed(() =>
    this.currency(
      this.restockLines().reduce((sum, line) => sum + line.value, 0),
    ),
  );
