...
import { tablerChevronDown, tablerClipboardCheck, tablerDatabase, tablerTruckDelivery } from '@ng-icons/tabler-icons';
...
export class InventoryActionModalComponent {
  ...
  protected readonly currentLocationStock = computed(() => this.data.item.locations[this.location()]);
  ...
  protected canSave(): boolean {
    if (this.data.mode === 'transfer') {
      return this.quantity() > 0 && this.from() !== this.to() && this.quantity() <= this.fromStock();
    }
